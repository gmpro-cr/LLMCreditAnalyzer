        ...
        {isEditing && !section.isLocked ? (
          <div className="flex flex-col bg-background">
            <Textarea
              ...
              autoFocus
            />
            <div className="flex justify-end gap-2 p-3 bg-muted/20 border-t">
    ...
    <div className="flex flex-col h-full animate-in fade-in duration-500 bg-muted/20">
      ...
      <div className="bg-background border-b px-8 py-5 shrink-0 sticky top-0 z-20">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">{caseData.borrowerName}</h1>
            <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
              <span className="flex items-center gap-1.5"><FileText className="h-4 w-4" /> {getFacilityTypeLabel(caseData.facilityType)}</span>
              <span className="flex items-center gap-1.5">₹ {formatCurrency(caseData.facilityAmount)}</span>
              <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> Created {formatDate(caseData.createdAt)}</span>
              {caseData.cin && <span>CIN: {caseData.cin}</span>}
            </div>
          </div>
        ...
        <div className="mt-6 flex items-center gap-4 max-w-xl">
          <div className="flex-1">
            <div className="flex justify-between text-xs font-medium mb-1.5">
              ...
              <span>{caseData.memoProgress}%</span>
            </div>
